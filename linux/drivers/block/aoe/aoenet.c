...
tx(int id)
{
	...
	while ((skb = skb_dequeue(&skbtxq))) {
		...
		if (current->flags & PF_MEMALLOC)
			...
			changef = ah->aflags & AOEAFL_WRITE;
		if (changef)
			current->flags |= PF_MEMALLOC;
		ifp = skb->dev;
		if (dev_queue_xmit(skb) == NET_XMIT_DROP && net_ratelimit())
			printk(KERN_WARNING
				"aoe: packet could not be sent on %s.  %s\n",
				ifp ? ifp->name : "netif",
...
aoenet_rcv(struct sk_buff *skb, struct net_device *ifp, struct packet_type *pt, struct net_device *orig_dev)
{
	...
	if (h->verfl & AOEFL_ERR) {
		...
		if (net_ratelimit())
			printk(KERN_ERR
				"aoe: error packet from %d.%d@%s; ecode=%d '%s'\n",
				be16_to_cpu(get_unaligned(&h->major)), h->minor,
				skb->dev->name, h->err, aoe_errlist[n]);
		goto exit;
	}
	...
	default:
		...
		printk(KERN_INFO
			"aoe: warning: unknown aoe command type 0x%02x\n",
			h->cmd);
		break;
	}
	if (skb)
exit:		dev_kfree_skb(skb);
	return 0;
}
